...
import { AiBuildPrompt } from "./ai-build-prompt"
...
export function OnboardingLandingScreen({ name, workspace }: { name: string; workspace: string }) {
    ...
    <div className="flex h-svh flex-col bg-background">
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="hidden w-56 shrink-0 flex-col border-r border-border px-3 py-4 sm:flex">
          ...
          <p className="px-2.5 text-xs text-muted-foreground/60">No pins!</p>

          <div className="mt-3 flex items-center gap-1.5 px-2.5 py-1">
            <Folder className="h-3.5 w-3.5 text-muted-foreground" />
            <span className="text-xs font-medium text-muted-foreground">Folders</span>
          </div>
          <p className="px-2.5 text-xs text-muted-foreground/60">No folders yet</p>
        ...
        <main className="flex-1 overflow-y-auto px-8 py-6">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                className="flex shrink-0 items-center gap-1 rounded-[var(--radius)] px-3.5 py-1.5 text-xs font-medium text-primary-foreground transition-opacity hover:opacity-90"
          ...
          <div className="mx-auto max-w-2xl pt-12">
            <div className="relative">
              <AiBuildPrompt highlighted={showCallout} />
              {showCallout && (
                <button
                  onClick={() => setShowCallout(false)}
                  className="absolute -bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold text-white shadow-sm transition-opacity hover:opacity-90"
