...
interface ProjectViewProps {
  ...
  tips?: ReactNode
  /** Rendered at the very end of the page content, after the response table. */
  afterTable?: ReactNode
  /** Rendered pinned to the bottom of the sidebar, e.g. a BackButton out of the prototype. */
  sidebarFooter?: ReactNode
  /**
   * When provided, adds "New Page" entry points to the sidebar (see ProjectViewSidebar) and a
   * "Page access | Page options | + New Page" row above the hero banner.
   */
  onAddPage?: () => void
  /** Anchored under the "+ New Page" button in the row above, e.g. a dropdown setup panel. */
  addPageMenu?: ReactNode
  /** Rendered at the very top of the scrollable content, above the hero banner. */
  ...
  return (
    <div className="flex h-full bg-background">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {onAddPage && (
          <div className="flex items-center gap-4 border-b border-border px-5 py-2 text-xs font-medium text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <Lock className="h-3.5 w-3.5" /> Page access
            </span>
            <span className="flex items-center gap-1.5">
              <Settings className="h-3.5 w-3.5" /> Page options
            </span>
            <div className="relative ml-auto">
              <button onClick={onAddPage} className="flex items-center gap-1.5 text-primary">
